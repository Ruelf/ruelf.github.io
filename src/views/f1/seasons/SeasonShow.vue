...
<template>
    <div class="flex flex-col gap-4 px-2 sm:px-0">
        <div class="flex items-center gap-4">
            <PrimaryButton
                :disabled="season <= config.f1.seasons.min"
            ...
            <PrimaryButton
                :disabled="season >= config.f1.seasons.max"
                @click="nextSeason"
            >
        ...
        <TabGroup
            :selected-index="
                tabs.findIndex(
                    (tab) => tab.route === router.currentRoute.value.name,
                )
            "
            @change="changeTab"
            class="mb-4"
            ...
            <TabList class="flex gap-4">
                <Tab v-for="tab of tabs" :key="tab.name" v-slot="{ selected }">
                    <div
                        ...
                        :class="selected ? 'bg-gray-600' : 'bg-gray-800'"
                    >
                        {{ tab.name }}
                    </div>
                </Tab>
